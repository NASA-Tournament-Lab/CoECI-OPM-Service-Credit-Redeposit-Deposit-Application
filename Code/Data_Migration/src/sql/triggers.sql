...
CREATE OR REPLACE FUNCTION opm.calculation_version_func() RETURNS TRIGGER AS $$
    BEGIN
    	  IF (TG_OP = 'UPDATE') OR (TG_OP = 'INSERT') THEN
            INSERT INTO opm.calculation_version_history(id, deleted, name, calculation_date, calculation_result_id, version, line_number, account_id, action)
            VALUES (NEW.id, NEW.deleted, NEW.name, NEW.calculation_date, NEW.calculation_result_id, NEW.version, NEW.line_number, NEW.account_id, substring(TG_OP,1,1));
            RETURN NEW;
        ELSIF (TG_OP = 'DELETE') THEN
            INSERT INTO opm.calculation_version_history(id, deleted, name, calculation_date, calculation_result_id, version, line_number, account_id, action)
            VALUES (OLD.id, OLD.deleted, OLD.name, OLD.calculation_date, OLD.calculation_result_id, OLD.version, OLD.line_number, OLD.account_id, substring(TG_OP,1,1));
            RETURN OLD;
        END IF;
    END;
$$ LANGUAGE plpgsql;
