...
public class AccountServiceImpl extends BaseService implements AccountService {
    ...
    public void updateInterest(long accountId) throws OPMException {
        ...
        try {
            ...
            if (calculationDate.before(today)) {
                ...
                CalculationResult result = calculationExecutionService.runCalculation(latestVersion.getCalculations(), new Date());

                // Save the update calculation version
                latestVersion.setCalculationResult(result);
                latestVersion.setCalculationDate(new Date());
                saveCalculationVersion(accountId, latestVersion);
                ...
                for (Dedeposit deposit : result.getDedeposits()) {
                    ...
                    for (Calculation calculation : latestVersion.getCalculations()) {
     ...
     *
     * @throws OPMConfigurationException if calculationExecutionService is null
     * @since 1.2 (OPM - Release I Assembly 1.0)
     */
    @PostConstruct
    ...
    private static DepositType getServiceCategory(Calculation calculation) {
        ...
        if (periodType.equals("DEPOSIT") && retirementType.equals("FERS")) {
            ...
            if (calculation.getEndDate().before(splitDate1082)) {
                ...
                && "PEACE CORPS/VISTA".equals(serviceType)) {
